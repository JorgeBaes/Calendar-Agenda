...
function request_delete_task(task_id){
   const evt_index = tag.tasks.findIndex( el => el.id == task_id)
   if(evt_index != -1){
       delete_task(tag.tasks[evt_index])
   }
...
////EDIT
/////
let current_task_id_editing = ""
function open_edit_task(task_id){
   current_task_id_editing = task_id

   $(`#modal-edit-task`).modal('show') 
   $(`#modal-display-task`).modal('toggle') 
   const evt_index = tag.tasks.findIndex( el => el.id == task_id)
   const task_to_be_edited = tag.tasks[evt_index]

   input_editor_task_name.value = task_to_be_edited.name
   input_editor_task_date.valueAsDate = new Date(task_to_be_edited.date)
   input_editor_task_text.value = task_to_be_edited.text
}

function request_task_edit(){
   const evt_index = tag.tasks.findIndex( el => el.id == current_task_id_editing)
   if(evt_index != -1){
       const task_to_be_edited = tag.tasks[evt_index]
       const edited_task = {
           user_id: user_id, 
           tag_id: tag_id,
           id:task_to_be_edited.id,
           name:input_editor_task_name.value,
           date:new Date(input_editor_task_date.value),
           text:input_editor_task_text.value
        }
       edit_task(task_to_be_edited,edited_task)
...
function open_task_display(task_id){
   const task_to_be_displayed = tag.tasks[tag.tasks.findIndex( el => el.id == task_id)]
   
   $(`#modal-display-task`).modal('show')
   display_task_title.innerText = task_to_be_displayed.name
   display_task_text.innerText = task_to_be_displayed.text
   display_task_date.innerText = convert_date_to_string(task_to_be_displayed.date)

   document.getElementById("display-task-editor-spot").innerHTML = 
   `
   <span onclick="open_edit_task('${task_id}')">
   ...
   `
   <span onclick="request_delete_task('${task_id}')">
